...
// Initialisation des éléments de fond animé pour desktop uniquement
document.addEventListener('DOMContentLoaded', function() {
    // Détection des appareils mobiles
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || window.innerWidth < 768;
    
    // Initialiser le fond animé uniquement pour desktop
    if (!isMobile) {
        initDesktopBackground();
    }
});

// Fonction pour initialiser le fond animé sophistiqué pour desktop
function initDesktopBackground() {
    // Créer les lignes horizontales néon
    const neonLinesContainer = document.querySelector('.neon-lines');
    if (neonLinesContainer) {
        // Créer 5 lignes horizontales à différentes positions
        for (let i = 0; i < 5; i++) {
    ...
    // Créer les cercles lumineux
    const glowCirclesContainer = document.querySelector('.glow-circles');
    if (glowCirclesContainer) {
        // Créer 3 grands cercles lumineux
        for (let i = 0; i < 3; i++) {
            const circle = document.createElement('div');
            circle.className = 'glow-circle';
            
            // Taille aléatoire entre 200px et 400px
            ...
            glowCirclesContainer.appendChild(circle);
        }
    }
    
    // Créer les points lumineux scintillants
    const sparklesContainer = document.querySelector('.sparkles');
    if (sparklesContainer) {
        // Créer 50 points lumineux
        for (let i = 0; i < 50; i++) {
