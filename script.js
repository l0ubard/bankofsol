...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof particlesJS !== 'undefined') {
        // Configuration ultra-légère pour mobile pour éviter les crashs
        if (isMobile) {
            particlesJS('particles-js', {
                particles: {
                    number: { value: 15, density: { enable: true, value_area: 1000 } }, // Nombre de particules très réduit
                    color: { value: "#00c2ff" },
                    shape: {
                        type: "circle",
                    ...
                    opacity: {
                        value: 0.3,
                        random: true,
                        anim: { enable: false } // Désactive l'animation d'opacité pour économiser des ressources
                    },
                    size: {
                        value: 2,
                        random: true,
                        anim: { enable: false } // Désactive l'animation de taille pour économiser des ressources
                    },
                    line_linked: {
                        enable: true, // Garder les lignes mais avec des paramètres très légers
                        distance: 150, // Distance plus grande = moins de lignes
                        color: "#00c2ff",
                        opacity: 0.2, // Opacité très réduite
                        width: 0.5 // Largeur réduite
                    },
                    move: {
                        enable: true,
                        speed: 0.5, // Vitesse très réduite
                        direction: "none",
                        random: true,
                        straight: false,
                        out_mode: "out",
                ...
                interactivity: {
                    detect_on: "canvas",
                    events: {
                        onhover: { enable: false }, // Désactive l'interactivité au survol pour économiser des ressources
                        onclick: { enable: false }, // Désactive l'interactivité au clic pour économiser des ressources
                        resize: true
                    }
                },
    ...
    if (splineViewer) {
        ...
        // Si l'élément spline-viewer n'existe pas, considérer l'animation comme chargée
        console.log('No 3D animation found, marking as loaded');
        animationsLoaded = true;
    }
    
    // Set shorter timeouts for mobile to avoid long loading screens
    ...
    function initializeMainContent() {
        ...
        // Initialize AOS with ultra-optimized settings for mobile
        if (typeof AOS !== 'undefined') {
            try {
                AOS.init({
                    duration: isMobile ? 400 : 1000, // Durée très courte sur mobile
                    easing: 'ease', // Easing le plus simple
                    once: true, // Toujours jouer les animations qu'une seule fois pour économiser les ressources
                    mirror: false,
                    disable: isMobile, // Désactiver complètement AOS sur mobile pour éviter les crashs
                    anchorPlacement: 'top-bottom',
                    throttleDelay: isMobile ? 200 : 50 // Augmenter considérablement le délai de throttle sur mobile
                });
            } catch (e) {
        ...
        // Add parallax effect to hero section - désactivé sur mobile pour éviter les crashs
        if (!isMobile) {
...
// Add particle background effect with mobile optimization - désactivé sur mobile pour éviter les crashs
document.addEventListener('DOMContentLoaded', function() {
    // Détection des appareils mobiles
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || window.innerWidth < 768;
    
    // Ne pas ajouter de particules supplémentaires sur mobile pour éviter les crashs
    if (!isMobile) {
